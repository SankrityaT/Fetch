...
console.log('warnings and the classic renderer')
{
  const film = L.merge(L.defaults(), { preset: 'film' }).look
  is('fields nothing draws yet are named', L.warnings(film).some(w => /grain\.film/.test(w) && /not drawn/.test(w)), true)
...
console.log('v1 documents')
{
  ...
  is('the sound moves to audio', d.audio, { denoise: true, loudnorm: false, gain: 3, music: 'warm' })
