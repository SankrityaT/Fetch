...
seeded by the frame index, or, where the look asks for a loop, by the frame's place
inside it (`gl.js`, `loopIndex`, off `spec.loop`), which is still a function of the
frame's own time, so any frame still draws alone. Every frame an export draws is
already inside the loop, so the file is the same either way; what the switch buys is
that a player counting frames on past the end, which the stage does when it plays the
clip round again, draws the frames the file holds rather than a second cycle of fresh
noise. Whether an edit can loop at all is `gl.js` `loopCheck`, off the plan and before
any pixel: it reads what varies at the two ends and names what stops the last frame
handing over to the first, each fault with the fix. Motion blur is analytic (its shutter reads the derivative of
...
| 8 | steps, arrows and cursor | the content target | while they show | Canvas2D pictures drawn once per size (`pic`): step badges popping in and shrinking away, riding a lifted card; arrows standing outside the box they point at, growing about their own tip so the pop never carries the head over it, with the side picked against what the zoom shows at the arrow's own hold, sized through that same window and the length cut to the room it leaves, and a side whose tip would fall outside it, or whose tip that window would push off the box it aims at, refused in favour of one that can be seen; the agent's own cursor pressing on clicks, its gold ripple, the Biscuit tag and badge, or, on a touch take, the disc where a finger went, a 44 point target measured through the device's own screen, appearing, pressing and gone, with nothing on the glass between taps. Mipmapped after, so a zoom samples them like the take. None of it was on the screen that was recorded, so each carves its own coverage out of the mask in that target's alpha and the grade leaves it alone. Its coverage, not its alpha: a badge's drop shadow and a caption's blurred glyph cloud lie over the recording at a third of an alpha, and the recording under them is still the recording. Carved by the whole of that alpha, a look that takes the colour out of a take left a soft coloured halo ninety pixels wide round every badge, the agent's cursor and every caption over the picture. |
...
- Encode: x264 is one of three sinks now. VP9 writes a WebM, on the classic renderer's own
  settings written out, because a quality name is a promise about size and what changed this
  round is the picture going in rather than what the encoder is told about it. A GIF gets a
  palette pass at the sink and not in the compositor: picking a clip's colours is a function
  of all of its frames at once and every pass here is a function of one, and the sink is where
  the finished stream goes past in order anyway. A GIF is also drawn without the ground's
  tooth and the film's grain, which 256 colours cannot carry and which cost five times the
  file, and at a rate its own centisecond clock can hold. x264, fed packed NV12 over loopback TCP (a pipe from the renderer took 144 fps,
...
- A capture: no decode at all. A screenshot is a take of one frame, so a shot is a
  ...
  second renderer and no pass in the table above that knows a shot from a take: the
  document lends the plan a four second clock and draws the instant in the middle of it
  (`ui/shot.js SPAN` and `HOLD`), where every arrival has landed and nothing has begun
  to leave, and the fields one frame cannot mean are pinned at the moment of projection
  rather than edited out of the look (`STILL_PINS`). No pass reads the previous frame,
  which for a still is trivially true and stays true because the shared code is the
  same code.
